...
def main():
    ...
    # ============================================================
    # WALK-FORWARD REGIME GATE BACKTEST
    # ============================================================

    print()
    print("=" * 100)
    print("WALK-FORWARD REGIME GATE BACKTEST")
    print("=" * 100)

    gate_df = base_regime.copy().sort_values(["Date", "Ticker"]).reset_index(drop=True)
    unique_dates = pd.DatetimeIndex(sorted(gate_df["Date"].dropna().unique()))
    date_to_idx = {pd.Timestamp(d): i for i, d in enumerate(unique_dates)}
    gate_df["_date_idx"] = gate_df["Date"].map(date_to_idx)
    TARGET_HORIZON = 30
    gate_df["_label_available_idx"] = gate_df["_date_idx"] + TARGET_HORIZON

    for c in ["MarketRegime", "Momentum20Bucket", "Momentum60Bucket", "VolatilityBucket", "SMA200Bucket"]:
        gate_df[c] = gate_df[c].astype("string").fillna("NA")
    ...
    MIN_HISTORY = 500
    gate_configs = {
        "LOOSE": {"auc": 0.51, "bal": 0.50},
        "NORMAL": {"auc": 0.53, "bal": 0.51},
        "STRICT": {"auc": 0.55, "bal": 0.52},
    }

    decisions = []
    for di, current_date in enumerate(unique_dates):
        today = gate_df[gate_df["_date_idx"] == di]
        hist = gate_df[gate_df["_label_available_idx"] <= di]
        level_maps = {}
        if len(hist) >= MIN_HISTORY:
            for level_name, cols in regime_levels:
                gm = {}
                for key, g in hist.groupby(cols, observed=True):
                    if len(g) < MIN_HISTORY:
                        continue
                    if not isinstance(key, tuple): key = (key,)
                    gm[tuple(str(x) for x in key)] = {
                        "Samples": len(g),
                        "AUC": safe_auc(g["actual"].values, g["probability"].values),
                        "BalancedAccuracy": balanced_accuracy_score(g["actual"], g["prediction"]),
                    }
                level_maps[level_name] = (cols, gm)

        for idx, row in today.iterrows():
            chosen_level, chosen = "NONE", None
            for level_name, cols in regime_levels:
                if level_name not in level_maps: continue
                gm = level_maps[level_name][1]
                key = tuple(str(row[c]) for c in cols)
                if key in gm:
                    chosen_level, chosen = level_name, gm[key]
                    break
            auc_hist = chosen["AUC"] if chosen else np.nan
            bal_hist = chosen["BalancedAccuracy"] if chosen else np.nan
            rec = {"_row_id": idx, "GateLevel": chosen_level,
                   "GateHistorySamples": chosen["Samples"] if chosen else 0,
                   "GateHistoricalAUC": auc_hist,
                   "GateHistoricalBalancedAccuracy": bal_hist}
            for name, cfg in gate_configs.items():
                rec[name] = bool(chosen and pd.notna(auc_hist) and auc_hist >= cfg["auc"] and bal_hist >= cfg["bal"])
            decisions.append(rec)
        if di % 250 == 0 or di == len(unique_dates)-1:
            print(f"Gate date {di+1}/{len(unique_dates)} | {pd.Timestamp(current_date).date()}")

    gd = pd.DataFrame(decisions).set_index("_row_id")
    gate_df = gate_df.join(gd, how="left")

    def eval_gate(name, mask):
        g = gate_df[mask].copy()
        if g.empty:
            return {"Model": name, "Samples": 0, "Coverage": 0.0, "Accuracy": np.nan, "BalancedAccuracy": np.nan, "AUC": np.nan}
        return {"Model": name, "Samples": len(g), "Coverage": len(g)/len(gate_df),
                "Accuracy": accuracy_score(g["actual"], g["prediction"]),
                "BalancedAccuracy": balanced_accuracy_score(g["actual"], g["prediction"]),
                "AUC": safe_auc(g["actual"].values, g["probability"].values)}

    rows=[eval_gate("BASE_ALL", pd.Series(True,index=gate_df.index))]
    for n in ["LOOSE","NORMAL","STRICT"]:
        rows.append(eval_gate("REGIME_"+n, gate_df[n].fillna(False)))
    gate_summary=pd.DataFrame(rows)
    disp=gate_summary.copy()
    for c in ["Coverage","Accuracy","BalancedAccuracy"]: disp[c]*=100
    print("\n"+"-"*100+"\nBASE vs REGIME-GATED BASE\n"+"-"*100)
    print(disp.to_string(index=False,formatters={"Coverage":lambda x:f"%{x:.2f}","Accuracy":lambda x:f"%{x:.2f}","BalancedAccuracy":lambda x:f"%{x:.2f}","AUC":lambda x:f"{x:.4f}" if pd.notna(x) else "NaN"}))

    gate_df["Year"] = gate_df["Date"].dt.year
    yr=[]
    for year in sorted(gate_df["Year"].unique()):
        y=gate_df[gate_df["Year"]==year]
        for n in ["BASE_ALL","LOOSE","NORMAL","STRICT"]:
            g=y if n=="BASE_ALL" else y[y[n].fillna(False)]
            if g.empty: continue
            yr.append({"Year":int(year),"Gate":n,"Samples":len(g),"Coverage":len(g)/len(y),
                       "Accuracy":accuracy_score(g["actual"],g["prediction"]),
                       "BalancedAccuracy":balanced_accuracy_score(g["actual"],g["prediction"]),
                       "AUC":safe_auc(g["actual"].values,g["probability"].values)})
    yearly_gate=pd.DataFrame(yr)
    yd=yearly_gate.copy()
    for c in ["Coverage","Accuracy","BalancedAccuracy"]: yd[c]*=100
    print("\n"+"-"*100+"\nYEAR-BY-YEAR WALK-FORWARD GATE\n"+"-"*100)
    print(yd.to_string(index=False,formatters={"Coverage":lambda x:f"%{x:.2f}","Accuracy":lambda x:f"%{x:.2f}","BalancedAccuracy":lambda x:f"%{x:.2f}","AUC":lambda x:f"{x:.4f}" if pd.notna(x) else "NaN"}))

    levels=gate_df["GateLevel"].value_counts(dropna=False).rename_axis("GateLevel").reset_index(name="Samples")
    levels["Rate"]=levels["Samples"]/len(gate_df)
    ld=levels.copy(); ld["Rate"]*=100
    print("\n"+"-"*100+"\nGATE BACKOFF LEVEL USAGE\n"+"-"*100)
    print(ld.to_string(index=False,formatters={"Rate":lambda x:f"%{x:.2f}"}))

    gate_summary.to_csv("data/base_30d_walkforward_regime_gate_summary.csv",index=False)
    yearly_gate.to_csv("data/base_30d_walkforward_regime_gate_yearly.csv",index=False)
    levels.to_csv("data/base_30d_walkforward_regime_gate_levels.csv",index=False)
    gate_df.to_csv("data/base_30d_walkforward_regime_gate_predictions.csv",index=False)
